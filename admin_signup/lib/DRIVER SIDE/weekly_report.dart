...
class WeeklyReportScreen extends StatefulWidget {
  ...
  const WeeklyReportScreen({Key? key, required this.email}) : super(key: key);
...
class _WeeklyReportScreenState extends State<WeeklyReportScreen> {
  Map<String, List<int>> eventData = {
    'Hard Braking': List.filled(0, 0),
    'Sharp Turns': List.filled(0, 0),
    'Over Speeding': List.filled(0, 0),
  };
  ...
  Future<void> fetchWeeklyData() async {
    final response = await http
        .get(Uri.parse('$driverapiurl/driver-weekly-events/${widget.email}'));
